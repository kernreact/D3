async function drawScatter() {
  ...
  const topHistogramBounds = bounds
    ...
    .attr(
      'transform',
      `translate(0, ${-dimensions.histogramHeight - dimensions.histogramMargin})`
    );
  ...
  function onVoronoiMouseEnter(e, datum) {
    ...
    const tooltipY = yScale(yAccessor(datum))
      ...
      - 6 // bump up so it doesn't overlap with out hover circle
  ...
  function onLegendMouseMove(e) {
    ...
    const isDayWithinRange = d => {
      ...
      if (getYear(minDateToHighlight) < colorScaleYear) {
        // if dates wrapo around to previos year,
        // check if this date is after the nin date
        return (
